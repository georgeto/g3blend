...
class ReadWriteKaitaiStruct(KaitaiStruct):
    ...
    def _write__seq(self, io):
        ...
        # Need to invoke check as that sets the values of field with fixed content.
        self._check()

    _T = TypeVar('_T')

    def cst(self, data_type: Type[_T]) -> _T:
        """Create child object of given type, correctly setting its _io, _parent and _root property.
        """
        return data_type(_io=self._io, _parent=self, _root=self._root)
...
class KaitaiStream(object):
    ...
    packer_f4x3le = struct.Struct('<fff')
    ...
    def read_f4le_x3(self):
        return KaitaiStream.packer_f4x3le.unpack(self.read_bytes(12))
    ...
    def _ensure_bytes_left_to_write(self, n, pos):
        ...
        # If size is zero we are writing to a file, so skip the check.
        if self._size == 0:
            return
